...
class StickyRoles:
    ...
    @stickyroles.command(name="jail", pass_context=True)
    async def _jail(self, ctx, user:discord.Member, role: discord.Role):
        """Add a role to be reapplied to a user once he rejoin the server"""
        server = ctx.message.server
        settings = self.db[server.id]
        if user.id not in settings["to_reapply"]:
            settings["to_reapply"][user.id] = []
        settings["to_reapply"][user.id].append(role.id)
        self.save()
        await self.bot.say("Done. If he rejoins the server he will get the role {}.".format(role.name))
